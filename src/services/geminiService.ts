  ...
  while (attempt < MAX_RETRIES) {
    try {
      ...
      if (error instanceof GoogleGenerativeAIFetchError && error.status === 503) {
        ...
        console.log(`Gemini API 503 에러. ${delay}ms 후 재시도합니다... (시도 ${attempt}/${MAX_RETRIES})`);
...
export async function analyzeScreenshot(base64Image: string, visionDeficiency?: string) {
  return retryWithBackoff(async () => {
    try {
      ...
      const imageParts = [{ inlineData: { mimeType: 'image/png', data: base64Image.replace('data:image/png;base64,', '') } }];
